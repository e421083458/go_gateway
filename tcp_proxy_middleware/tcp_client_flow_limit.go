...
func TcpClientFlowLimitMiddleware() func(c *TcpSliceRouterContext) {
	return func(c *TcpSliceRouterContext) {
		tmp := c.Get("service_detail")
		if tmp == nil {
			c.conn.Write([]byte("TcpClientFlowLimitMiddleware get service_detail error"))
			c.Abort()
			return
		}
		...
		if clientIPLimit > 0 {
			limiter, err := public.FlowLimiterHandler.GetLimiter(
				...
				int(clientIPLimit*3))
			if err != nil {
				c.conn.Write([]byte("HttpClientFlowLimitMiddleware get GetLimiter error"))
				c.Abort()
				return
			}
			if !limiter.Allow() {
				fmt.Println("not allow")
				c.conn.Write([]byte(fmt.Sprintf("client rate limiting %v,%v", limiter.Limit(), limiter.Burst())))
				c.Abort()
				return
			}
		}
